...
test_normalized_pos(WT_CONNECTION *conn)
{
    ...
    for (uint64_t key = 0; key < NUM_KEYS; key++) {
        ...
        npos = __wt_page_npos(wt_session, page_ref, 0, path_str, &path_str_offset, 1024);
        ...
        testutil_assertfmt(npos >= prev_npos,
          "Page containing key " PRIu64 "\n has npos (%f) smaller than the page of key " PRIu64
          ", (%f)",
          key, npos, key - 1, prev_npos);
        prev_npos = npos;

        /* Now find which page npos restores to. We haven't modified the Btree so it should be the
         * exact same page */
        WT_WITH_DHANDLE(wt_session, dhandle,
          testutil_check(__wt_page_from_npos(wt_session, &restored_page_ref,
            WT_READ_EVICT_READ_FLAGS, WT_READ_EVICT_WALK_FLAGS, npos)));
